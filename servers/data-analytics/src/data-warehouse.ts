...
export class DataWarehouseService {
  ...
  constructor(configPath: string = './data/data-warehouse') {
    ...
    this.healthChecker = new HealthChecker("data-warehouse-service");
  }
...
export class DataWarehouseMCPServer extends StandardMCPServer {
    ...
    switch (name) {
      case 'create_warehouse':
        return { 
          content: [{ 
            type: 'text', 
            text: JSON.stringify(await this.dataWarehouseService.createWarehouse(args), null, 2) 
          }] 
        };
      case 'run_query':
        return { 
          content: [{ 
            type: 'text', 
            text: JSON.stringify(await this.dataWarehouseService.executeQuery(args.warehouseId, args.query), null, 2) 
          }] 
        };
      default:
